function [tp_gen,mdl]=bls_offset_noncounter_nonuniformPrior_self(ts,modelparams,tp,gen_model_type,savefolder)
mdl.type = 'noncounter';
if isempty(tp)
    %generate data
    ...
    tm = normrnd(ts, wm*(ts));
    te = fbls(ts, tm, wm);
    tp_gen = normrnd(te, wp*(te))+offset;
    mdl=[];
    return
end
...
tm = normrnd(ts, wmopt*(ts));
te = fbls(ts, tm, wmopt);
tp_gen = normrnd(te, wpopt*(te))+w_offset;
